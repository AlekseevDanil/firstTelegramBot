...
admin_id = ['874493573']
with open("files/app.txt", "r") as f:
    applications = f.read().replace('\n', '').strip()
sponsor = []
...
def stop_message(message):
    try:
        ...
        if str(message.chat.id) != group_id:
            if str(message.from_user.id) in admin_id:
                with open("files/app.txt", "w") as f:
                    f.write("False")
                with open("files/app.txt", "r") as f:
                    applications = f.read().replace('\n', '').strip()
